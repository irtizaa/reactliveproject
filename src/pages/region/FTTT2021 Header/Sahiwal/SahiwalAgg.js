...
export default class FTTT_SahiwalAgg extends React.Component {
    ...
    componentDidMount() {
        ...
        loadModules(["esri/tasks/support/Query", "esri/tasks/QueryTask", "esri/Graphic", "esri/layers/GraphicsLayer"
            ...
            .then(([Query, QueryTask, Graphic, GraphicsLayer]) => {
                ...
                let Sahiwal = document.getElementById('Sahiwal')
                Sahiwal.addEventListener('click', function () {
                    view.goTo({
                        target: [73.1114, 30.6682],
                        zoom: 12
                    })
                })
                ...
                query1.where = "City = 'Sahiwal' AND Stage = 'Pending' AND Project_Year = '2021'";

                queryTask.executeForCount(query1).then(function (results) {
                    document.getElementById('FTTT_Sahiwl_PendingAgg').innerHTML = results
                }); 
                ...
                let query2 = new Query({
                    ...
                    outFields: ["*"],
                    // where: "Stage = 'LitUp'"
                    where : "City = 'Sahiwal' AND Stage = 'LitUp' AND Project_Year = '2021'" 
                });
                queryTask.executeForCount(query2).then(function (results) {
                    document.getElementById('FTTT_Sahiwl_LitUpAgg').innerHTML = results
                });
            })
    }
