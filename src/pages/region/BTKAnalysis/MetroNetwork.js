    ...
    loadModules(["esri/layers/FeatureLayer","esri/widgets/Search", "esri/tasks/support/Query", "esri/tasks/QueryTask",
     "esri/layers/GraphicsLayer", "esri/geometry/geometryEngine", "esri/Graphic", 'esri/widgets/Expand', "esri/geometry/Extent"
      ...
      .then(([FeatureLayer, Search, Query, QueryTask, GraphicsLayer, geometryEngine, Graphic, Expand, Extent]) => {  
        ...
        var fiberurl =
          "http://103.31.82.102:6080/arcgis/rest/services/Projects/BahriaTown/MapServer/7"; 
        ...
        var phase = document.getElementById("MetroPhaseId");
        ...
        var phaselayer = new FeatureLayer({
        ...
        view.when(function () {
          return phaselayer.when(function () {
            var query = phaselayer.createQuery();
            return phaselayer.queryFeatures(query);
          });
        })
          .then(getValues)
        ...
        function addToSelect(values) {
          ...
          return setWellsDefinitionExpression(phase.value);
        }

        function setWellsDefinitionExpression(newValue) {
          phaselayer.definitionExpression = "Phase_Number = '" + newValue + "'";

          let queryPhase = new Query({
            returnGeometry: true,
            outSpatialReference: { wkid: 4326 },
            where: phaselayer.definitionExpression

          });
          phaselayer.queryFeatures(queryPhase).then(function (results) {  
          ...
          if (!phaselayer.visible) {
            phaselayer.visible = true;
          }
        ...
        function queryForWellGeometries() {
          var districtQuery = phaselayer.createQuery();

          return phaselayer.queryFeatures(districtQuery).then(function (response) {
        ...
        phase.addEventListener("change", selectPhase)
        
        function selectPhase(event){
        ...
        function fortyeightcore() {
          ...
          queryDC.outFields = ["FOC_ID", "CalculatedLength", "Survey_Date", "Core_No"];
    ...
    return (
      <div>
        <div id="infoDiv4">
          ...
          <br/>
          <strong>Select Phase Name</strong>
          <select id="MetroPhaseId">
