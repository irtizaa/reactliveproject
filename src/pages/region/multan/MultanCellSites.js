...
export default class MultanCellsites extends React.Component {
    ...
    componentDidMount() {
        ...
        loadModules(["esri/tasks/support/Query", "esri/tasks/QueryTask", "esri/Graphic", "esri/layers/GraphicsLayer"
            ...
            .then(([Query, QueryTask, Graphic, GraphicsLayer]) => {
                ...
                query1.where = "City = 'Multan' AND Stage = 'Pending'  AND Project_Year = '2019'"

                queryTask.executeForCount(query1).then(function (results) {
                    document.getElementById('M_PendingCellSite').innerHTML = results
                }); 
                ...
                let query2 = new Query({
                    ...
                    outFields: ["*"],
                    // where: "Stage = 'LitUp'"
                    where : "City = 'Multan' AND Stage = 'LitUp'  AND Project_Year = '2019'" 
                });
                queryTask.executeForCount(query2).then(function (results) {
                    document.getElementById('M_LitUpCellSite').innerHTML = results
                });
            })
    }
